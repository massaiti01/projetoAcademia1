...
public class EmailDAOImpl {
    ...
    public Boolean EnviarSenha(Object object) {
        Pessoa pessoa = (Pessoa) object;
        Properties props = new Properties();
        /**
         * Parâmetros de conexão com servidor Gmail
        ...
        try {
            ...
            Address[] toUser = InternetAddress //Destinatário(s)
                    .parse(pessoa.getEmailPessoa());

            message.setRecipients(Message.RecipientType.TO, toUser);
            message.setSubject("Senha Redefinida");//Assunto
            message.setText("Ola : " + pessoa.getEmailPessoa()+ "Sua Senha foi redefinida para "+pessoa.getSenhaPessoa());
            /**
             * Método para enviar a mensagem criada
             */
            Transport.send(message);
